...
//
// Calcula o frete pelo CEP de destino usando a API do Melhor Envio.
// Chamada pelo checkout.html sempre que o cliente digita o CEP.
//
// Body esperado (POST):
// { "cepDestino": "01001000", "items": [{ "quantity": 2 }, ...] }
//
// Resposta:
// { "options": [ { "service": "PAC", "price": 24.50, "days": 6 }, ... ] }
  ...
  try {
    ...
    if (!token || !originCep) {
      return {
        ...
        body: JSON.stringify({
          error:
            "Frete não configurado. Defina MELHORENVIO_TOKEN e STORE_ORIGIN_CEP nas variáveis de ambiente.",
        }),
      };
    }

    // Dimensões/peso padrão por item — ajuste conforme os produtos reais.
    // Uma versão mais avançada guardaria isso por produto no banco.
    ...
    // A resposta do Melhor Envio traz uma lista de transportadoras/serviços.
    // Filtramos só as que retornaram preço válido (sem erro).
    ...
    return {
      statusCode: 500,
      body: JSON.stringify({ error: "Erro interno ao calcular o frete." }),
    };
  }
};
